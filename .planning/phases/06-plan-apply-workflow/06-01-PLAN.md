...
- `func ComputeDiff(desired []DesiredInstance, running []string) []Change`
  ...
  - Modifications: disabled desired instances whose container_name IS in runningSet (action=modify, field "enabled" old=true new=false)
  - Also: enabled desired instances whose container_name IS in runningSet but instance was recently modified (for now, skip deep config diff — just detect enabled/disabled toggle)
...
`cd /home/priz/projects/devarch/api && go build ./internal/plan/...` compiles without errors.
  ...
  <done>
Plan types exist with JSON tags. ComputeDiff correctly identifies add/modify/remove from desired vs running inputs.
  </done>
</task>
