...
must_haves:
  truths:
    ...
    - "Effective config includes template dependencies as read-only (INST-05: dependencies visible but not overridable)"
  artifacts:
...
4. Merge with these semantics (from RESEARCH.md):
   ...
   - **Dependencies**: Template dependencies only — NOT overridable. Include template service_dependencies in the effective config response as-is. This covers INST-05: dependencies are visible in effective config but cannot be overridden by users (per CONTEXT.md decision "Dependency overrides NOT user-editable, wiring is Phase 8").
...
The `overrides_applied` field tells the UI which values came from overrides vs template. For key-based merges (env_vars, labels, config_files), list the specific keys that were overridden. For full-replacement types (ports, volumes, domains, healthcheck), use boolean. Dependencies are never in overrides_applied since they are template-only.

Wire route: `r.Get("/effective-config", instanceHandler.EffectiveConfig)`
  </action>
  <verify>`go build ./cmd/server` compiles. Test with curl: create instance, add port override, GET effective-config shows merged result with template data + override ports + template dependencies.</verify>
  <done>Effective config endpoint returns correctly merged template + overrides with override tracking metadata. Dependencies included from template only (INST-05).</done>
</task>
...
- Dependencies come from template only in effective config (INST-05)
...
- INST-05 covered: dependencies visible in effective config as template-only (not overridable)
