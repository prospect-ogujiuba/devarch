# Requirements Archive: v1.0 Stacks & Instances
...
- [x] **PLAN-01**: Plan endpoint returns structured preview of what will change (add/modify/remove)
- [x] **PLAN-02**: Apply endpoint executes plan with advisory lock per stack
- [x] **PLAN-03**: Apply flow: lock → ensure network → materialize configs → compose up
- [x] **PLAN-04**: Plan staleness detection (reject apply if stack changed since plan)
- [x] **PLAN-05**: Structured diff output in plan (+ add, ~ modify, - remove)

### Service Wiring

- [x] **WIRE-01**: Service export declarations on template catalog (name, type, port, protocol)
- [x] **WIRE-02**: Service import contracts on template catalog (what a service needs)
- [x] **WIRE-03**: Wiring cache table (service_instance_wires with partial unique constraints)
- [x] **WIRE-04**: Auto-wiring for unambiguous cases (one provider matches one consumer)
- [x] **WIRE-05**: Explicit contract-based wiring for ambiguous cases
- [x] **WIRE-06**: Wiring diagnostics in plan (missing/ambiguous required contracts)
- [x] **WIRE-07**: Env var injection from wires (DB_HOST, DB_PORT using internal DNS + container port)
- [x] **WIRE-08**: Consumer instance env overrides win over injected wire values

### Export/Import

- [x] **EXIM-01**: Export stack to devarch.yml (stack + instances + overrides; wires added in Phase 8)
- [x] **EXIM-02**: Import devarch.yml with create-update reconciliation
- [x] **EXIM-03**: Version field in devarch.yml format
- [x] **EXIM-04**: Secret redaction in exports (placeholders, not plaintext)
- [x] **EXIM-05**: Export → import → export round-trip stable (excluding ids/timestamps)
- [x] **EXIM-06**: Export includes resolved specifics: chosen host ports, image digests/pinned versions, template versions

### Secrets

- [x] **SECR-01**: Secrets encrypted at rest (AES-256-GCM)
- [x] **SECR-02**: Auto-generated encryption key in ~/.devarch/secret.key
- [x] **SECR-03**: Secret redaction in all API responses, plan output, compose previews
- [x] **SECR-04**: Encrypt before INSERT, decrypt after SELECT (transparent to app logic)

### Resource Limits

- [x] **RESC-01**: Resource limits per instance (CPU, memory)
- [x] **RESC-02**: Limits mapped to compose deploy.resources fields
- [x] **RESC-03**: Limits validated in plan output
...
- [x] **MIGR-03**: Migration 015: service_exports, service_import_contracts, service_instance_wires (Phase 8)
- [x] **MIGR-04**: Migration 016: secrets encryption fields (Phase 9)
- [x] **MIGR-05**: Migration 017: service_instance_resources (Phase 9)

### Bootstrap & Diagnostics

- [x] **BOOT-01**: `devarch init` — one-command project bootstrap from devarch.yml (pull images, create networks, apply config)
- [x] **BOOT-02**: `devarch doctor` — environment diagnostics (runtime running, permissions/SELinux, port conflicts, disk space, required tools)

### Lockfile

- [x] **LOCK-01**: Generate devarch.lock from resolved state (host ports, image digests, pinned versions, template versions)
- [x] **LOCK-02**: Lock validation on apply — warn/reject when runtime state diverges from lockfile
- [x] **LOCK-03**: Lock refresh command to update lockfile from current resolved state
...
| NETW-01 | Phase 4 | Complete |
| NETW-02 | Phase 4 | Complete |
| NETW-03 | Phase 4 | Complete |
| NETW-04 | Phase 4 | Complete |
...
| EXIM-01 | Phase 7 | Complete |
| EXIM-02 | Phase 7 | Complete |
| EXIM-03 | Phase 7 | Complete |
| EXIM-04 | Phase 7 | Complete |
| EXIM-05 | Phase 7 | Complete |
| EXIM-06 | Phase 7 | Complete |
...
| WIRE-04 | Phase 8 | Complete |
| WIRE-05 | Phase 8 | Complete |
| WIRE-06 | Phase 8 | Complete |
| WIRE-07 | Phase 8 | Complete |
| WIRE-08 | Phase 8 | Complete |
| MIGR-03 | Phase 8 | Complete |
| SECR-01 | Phase 9 | Complete |
| SECR-02 | Phase 9 | Complete |
| SECR-03 | Phase 9 | Complete |
| SECR-04 | Phase 9 | Complete |
| RESC-01 | Phase 9 | Complete |
| RESC-02 | Phase 9 | Complete |
| RESC-03 | Phase 9 | Complete |
...
*Last updated: 2026-02-09 — all v1 requirements complete (milestone done)*
