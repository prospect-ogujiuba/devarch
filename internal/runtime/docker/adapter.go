...
func (a *Adapter) Provider() string {
	return runtimepkg.ProviderDocker
}
...
func (a *Adapter) InspectWorkspace(ctx context.Context, desired *runtimepkg.DesiredWorkspace) (*runtimepkg.Snapshot, error) {
	if desired == nil {
		return nil, fmt.Errorf("docker inspect workspace: nil desired workspace")
	}
	...
	idsOutput, err := a.runner.Run(ctx, "docker", args...)
	if err != nil {
		return nil, err
	}
	...
	if desired.Network != nil {
		networkOutput, err = a.runner.Run(ctx, "docker", "network", "inspect", desired.Network.Name)
...
func (a *Adapter) StreamLogs(ctx context.Context, resource runtimepkg.ResourceRef, request runtimepkg.LogsRequest, consume runtimepkg.LogsConsumer) error {
	...
	output, err := a.runner.Run(ctx, "docker", args...)
	if err != nil {
		return err
	}
