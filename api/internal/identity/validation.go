...
// ValidateLabelKey returns error if key starts with "devarch." prefix (reserved for system labels)
func ValidateLabelKey(key string) error {
	if strings.HasPrefix(key, LabelPrefix) {
		return fmt.Errorf("label key %q is reserved: keys starting with %q are reserved for system use", key, LabelPrefix)
	}
	return nil
}
