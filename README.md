...
- Discovers workspace manifests from configured roots.
...
- [DB admin/proxy stack from scratch](docs/tutorial-db-admin-proxy.md) — starts at `devarch: command not found`, then uses native DevArch commands for PostgreSQL and catalog-template guidance for MariaDB/Adminer/Nginx Proxy Manager.
...
- workspace discovery, planning, applying, status, logs, exec, and restart
...
Those capabilities are outside the supported CLI surface.
