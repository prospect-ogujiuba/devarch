...
```bash
podman exec devarch-api go run ./cmd/import
```
...
The import command auto-detects `services-library/` at `/workspace/services-library` (container) or `../services-library` (local). Override with `-library-dir`, `-compose-dir`, or `-config-dir`.
...
| `cmd/import` | `go run ./cmd/import` | Import service templates from YAML |
...
| `cmd/verify-parity` | `go run ./cmd/verify-parity -compose-dir ../services-library/compose -project-root ..` | Verify imported data parity against compose templates |

Import flags:
- `-library-dir` — path to services-library root (auto-detected, contains `compose/` and `config/`)
- `-compose-dir` — path to compose YAML directory (overrides library-dir)
- `-config-dir` — path to config files directory (overrides library-dir)
...
Service templates live in `services-library/`, split into `compose/` (YAML templates) and `config/` (config files):
...
└── config/              # config files imported into DB
    ...
    ├── nginx-proxy-manager/  # custom nginx configs
    ├── prometheus/      # prometheus.yml
    └── ...              # 22 service config directories
```
