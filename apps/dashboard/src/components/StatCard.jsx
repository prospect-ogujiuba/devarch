...
export function StatCard({ label, value, runtime, colorClass }) {
  // Use custom color class if provided, otherwise use runtime color or default
  const finalColorClass = colorClass || (runtime ? getRuntimeTextClass(runtime) : 'text-slate-900 dark:text-slate-100')

  return (
    <div className="text-center">
      <div className={`text-3xl sm:text-4xl font-bold ${finalColorClass}`}>
        {value}
      </div>
...
export function ContainerStatsGrid({ stats }) {
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm p-4 sm:p-6">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
        <StatCard label="Total" value={stats.total} />
        <StatCard label="Running" value={stats.running} colorClass="text-green-600 dark:text-green-500" />
        <StatCard label="Stopped" value={stats.stopped} colorClass="text-gray-500 dark:text-gray-400" />
        <StatCard
          label="Avg CPU"
          value={stats.avgCpuPercentage ? `${stats.avgCpuPercentage.toFixed(1)}%` : '0%'}
          colorClass="text-blue-600 dark:text-blue-500"
        />
        <StatCard
          label="Memory"
          value={`${Math.round(stats.totalMemoryMb || 0)} MB`}
