...
- Document that obsolete entries are harmless/ignored, arbitrary child export is intentionally removed, local `.env` is never automatically rewritten, and existing MariaDB volumes require their initialization password.
- Document `DEVARCH_ENV_FILE` selection as a host control excluded from `.env.example`.
- Add a deterministic assertion that template keys are a subset of test-readable declared bootstrap allowlists.
...
2. Every active template key is accepted by at least one bootstrap; every omitted legacy key is identified in migration guidance by category; password/token examples are blank or unmistakably invalid.
