# Environment Configuration Cleanup — Initiative Specification r2

- Topic: `env-configuration-cleanup`
- Revision: 2
- Status: review-ready
- Created: 2026-09-11T16:25:00Z
- Predecessor: `.model-artifacts/initiatives/env-configuration-cleanup/specs/2026-09-11_1614-initiative-spec-r1.md`
...
5. Existing `.env` remains the optional default local path and existing supported aliases continue to work. An explicitly set, nonempty `DEVARCH_ENV_FILE` selects a regular file and fails before mutation when invalid; it is a host control and is never loaded from dotenv.
...
- Recognized malformed values may identify only key/path in diagnostics; unrecognized malformed lines remain inert.
- Dotenv assignments replace inherited values only for recognized keys and are not automatically exported.
...
- Root `.env` remains optional and ignored by Git; `DEVARCH_ENV_FILE` provides deterministic explicit selection without becoming a template key.
...
- **AC8:** Duplicate recognized assignments use the last value; malformed recognized values fail without reflecting contents; malformed unrecognized lines are inert.
