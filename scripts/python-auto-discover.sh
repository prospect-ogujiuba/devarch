...
echo "🐍 Python Apps Auto-Discovery - DevArch"
echo "========================================"
echo "🔍 Scanning $APPS_DIR for Python applications..."
echo ""
...
for app_dir in "$APPS_DIR"/*/; do
    ...
    if [ "$has_manage_py" = true ]; then
        ...
        if [ -f "$app_dir/main.py" ]; then
            ...
            echo "   ⚠️  FastAPI app found but no main.py or app.py entry point"
            continue
        fi

    else
        echo "   ⚠️  Could not detect framework - skipping"
        continue
    fi
...
if [ $app_count -eq 0 ]; then
    echo "⚠️  No Python applications found"
    echo ""
    echo "💡 To create a Python app:"
    echo "   1. Use PyCharm IDE: File → New Project → Django/Flask"
    ...
    echo ""
    echo "✅ Supervisord config generated (empty)"
else
    echo "✅ Generated supervisord config for $app_count app(s)"
    ...
    for ((i=0; i<app_count; i++)); do
        ...
        echo "   App $((i+1)): localhost:$external_port → :$internal_port"
    done
    echo ""
fi
echo "========================================"
